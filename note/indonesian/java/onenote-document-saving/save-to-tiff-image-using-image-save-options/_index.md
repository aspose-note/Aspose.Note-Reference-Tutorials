...
description: Pelajari cara menyimpan dokumen OneNote sebagai file TIFF menggunakan
  kompresi TIFF JPEG, kompresi TIFF PackBits, atau CCITT Group 3 Fax di Java. Kendalikan
  kualitas gambar, ukuran file, dan mode warna dengan Aspose.Note.
linktitle: Save to TIFF Image Using TIFF JPEG Compression in OneNote
second_title: Aspose.Note Java API
title: Simpan ke Gambar TIFF dengan Kompresi TIFF JPEG di OneNote
...
# Simpan Gambar TIFF Menggunakan Kompresi TIFF JPEG di OneNote

## Pendahuluan

Dalam tutorial ini Anda akan menemukan **cara menyimpan dokumen OneNote ke file TIFF menggunakan kompresi TIFF JPEG** dan dua metode kompresi populer lainnya. Kami akan memandu Anda melalui pengaturan yang diperlukan, mengimpor paket Java yang diperlukan, dan memberikan kode langkah‑demi‑langkah yang jelas untuk setiap opsi kompresi. Pada akhir tutorial Anda akan dapat mengontrol **kualitas gambar TIFF**, mengurangi ukuran file, dan bahkan menghasilkan TIFF hitam‑putih untuk output bergaya faks.
...
- **PackBits** – Enkoding run‑length sederhana yang loss‑less; berguna untuk grafik dengan area seragam yang besar.  
- **CCITT Group 3 Fax** – Hanya hitam‑putih; sempurna untuk dokumen yang dipindai dan transmisi faks.  

Memilih kompresi yang tepat memungkinkan Anda memenuhi batasan penyimpanan tanpa mengorbankan fidelitas visual yang diperlukan untuk aplikasi Anda.

## Konversi One ke TIFF Menggunakan Aspose.Note
Jika tujuan Anda adalah **mengonversi OneNote ke TIFF**, tiga metode di bawah ini mencakup skenario paling umum. Setiap metode memuat file `.one`, mengonfigurasi `ImageSaveOptions`, dan menyimpan hasilnya sebagai file `.tiff`.
...
Pertama, bawa kelas Aspose.Note yang diperlukan ke dalam file Java Anda:
...
- `setQuality(93)` (opsional) menyetel kualitas gambar secara halus; nilai lebih rendah menghasilkan file yang lebih kecil.
...
## Ikhtisar Kompresi TIFF PackBits

PackBits adalah algoritma kompresi lossless yang bekerja paling baik untuk gambar dengan area warna solid yang luas. Ini sering disebut sebagai **tiff packbits compression** dalam dokumentasi.

## Langkah 2: Simpan ke TIFF Menggunakan Kompresi PackBits

Jika Anda memerlukan opsi loss‑less, PackBits adalah algoritma run‑length sederhana yang cocok untuk grafik dengan warna solid.
...
- `setTiffCompression(TiffCompression.PackBits)` mengganti metode kompresi.  
...
- `setTiffCompression(TiffCompression.Ccitt3)` menerapkan kompresi yang ditujukan untuk faks.
...
| **Warna tampak pudar** | Pastikan Anda tidak secara tidak sengaja mengatur `ColorMode.BlackAndWhite` ketika Anda memerlukan warna penuh. |
| **Kesalahan format gambar tidak didukung** | Pastikan Anda menggunakan versi terbaru Aspose.Note; build lama mungkin tidak memiliki enum kompresi tertentu. |
...
**T: Bisakah saya mengonversi tipe dokumen lain (misalnya PDF, DOCX) ke TIFF dengan opsi ini?**  
J: Ya. Meskipun Aspose.Note berfokus pada file OneNote, Aspose.PDF, Aspose.Words, dan perpustakaan lain menyediakan `ImageSaveOptions` serupa untuk format mereka.

**T: Bagaimana kompresi TIFF JPEG berbeda dari file JPEG standar?**  
J: Algoritma JPEGnya sama, tetapi data gambar berada di dalam kontainer TIFF, yang dapat menampung banyak halaman dan metadata yang lebih kaya.

**T: Apakah memungkinkan memproses batch banyak file `.one`?**  
J: Tentu. Iterasi melalui direktori, panggil salah satu dari tiga metode untuk setiap file, dan kumpulkan TIFF yang dihasilkan.

**T: Bisakah saya mengontrol DPI/resolusi TIFF output?**  
J: Ya. Gunakan `options.setResolution(int dpi)` sebelum menyimpan.

**T: Apakah Aspose.Note mendukung pemrosesan asynchronous?**  
J: API-nya bersifat sinkron, tetapi Anda dapat membungkus pemanggilan dalam `CompletableFuture` Java atau thread pool untuk eksekusi paralel.

## Kesimpulan

Anda kini memiliki toolkit **java tiff conversion** lengkap yang memungkinkan Anda menyimpan dokumen OneNote sebagai file TIFF menggunakan kompresi JPEG, PackBits, atau CCITT Group 3 Fax. Sesuaikan kualitas, mode warna, dan resolusi untuk memenuhi persyaratan **kualitas gambar TIFF** spesifik Anda, dan integrasikan metode ini ke dalam alur kerja batch untuk produktivitas maksimal.
