---
date: 2026-03-14
description: 了解如何在 Java 中使用 TIFF JPEG 压缩、TIFF PackBits 压缩或 CCITT Group 3 传真将 OneNote
  文档保存为 TIFF 文件。使用 Aspose.Note 控制图像质量、文件大小和颜色模式。
...
# 使用 TIFF JPEG 压缩在 OneNote 中保存为 TIFF 图像

## 介绍

在本教程中，您将了解 **如何使用 TIFF JPEG 压缩将 OneNote 文档保存为 TIFF 文件**以及另外两种流行的压缩方法。我们将逐步演示所需的设置，导入必要的 Java 包，并为每种压缩选项提供清晰的逐步代码。完成后，您将能够控制 **TIFF 图像质量**，减小文件大小，甚至生成用于传真式输出的黑白 TIFF。

## 快速答案
- **什么是 TIFF JPEG 压缩？** 一种有损压缩方法，可在保持视觉质量的同时减小 TIFF 文件大小。  
- **哪个库负责转换？** Aspose.Note for Java。  
- **我需要许可证吗？** 免费试用可用于测试；生产环境需要许可证。  
- **我可以更改图像质量吗？** 可以，在 `ImageSaveOptions` 上设置 `quality` 属性。  
- **批量转换可能吗？** 完全可以——遍历文档并应用相同的选项。

## 什么是 TIFF JPEG 压缩？

TIFF JPEG 压缩将图像数据存储在 TIFF 容器中，但使用 JPEG 的有损算法来压缩文件。当您需要在 **tiff 图像质量** 与更小文件大小之间取得平衡时，它是理想选择，尤其适用于网页或归档场景。
...
## 使用 Aspose.Note 将 One 转换为 TIFF

如果您的目标是 **将 OneNote 转换为 TIFF**，下面的三种方法涵盖了最常见的场景。每种方法都会加载 `.one` 文件，配置 `ImageSaveOptions`，并将结果保存为 `.tiff` 文件。
...
下面是完整的方法，加载 OneNote 文件并使用 JPEG 压缩将其保存为 TIFF。调整 `quality` 值（0‑100）以控制 **tiff 图像质量**。
...
- `ImageSaveOptions` 告诉 Aspose.Note 输出 TIFF 文件。  
- `setTiffCompression(TiffCompression.Jpeg)` 选择 JPEG 压缩。  
- `setQuality(93)`（可选）微调图像质量；较低的值会生成更小的文件。

### 在 Java 中使用 JPEG 压缩保存 TIFF 的方法
1. 将 `dataDir` 指向包含 `.one` 文件的文件夹。  
2. 在主方法或服务中调用 `SaveToTiffUsingJpegCompression()`。  
3. 生成的 `.tiff` 文件将出现在同一目录中。

## TIFF PackBits 压缩概述

PackBits 是一种无损压缩算法，最适用于具有大面积纯色区域的图像。文档中常将其称为 **tiff packbits compression**。
...
## 步骤 3：使用 CCITT Group 3 Fax 压缩（黑白 TIFF）

对于传真式文档，您通常需要 **黑白 TIFF**。CCITT Group 3 为单色图像提供高压缩率。
...
**A:** 可以。虽然 Aspose.Note 侧重于 OneNote 文件，但 Aspose.PDF、Aspose.Words 等库为其各自的格式提供类似的 `ImageSaveOptions`。

**Q: TIFF JPEG 压缩与标准 JPEG 文件有何不同？**  
**A:** JPEG 算法相同，但图像数据位于 TIFF 容器中，后者可以包含多页和更丰富的元数据。

**Q: 是否可以批量处理大量 `.one` 文件？**  
**A:** 完全可以。遍历目录，对每个文件调用任意一种方法，并收集生成的 TIFF。

**Q: 我可以控制输出 TIFF 的 DPI/分辨率吗？**  
**A:** 可以。在保存之前使用 `options.setResolution(int dpi)`。
...
现在，您拥有了一套完整的 **java tiff conversion** 工具包，可使用 JPEG、PackBits 或 CCITT Group 3 Fax 压缩将 OneNote 文档保存为 TIFF 文件。调整质量、颜色模式和分辨率，以满足您特定的 **tiff 图像质量** 要求，并将这些方法集成到批量工作流中，以实现最高生产力。
